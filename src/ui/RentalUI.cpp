...
void render_rental_menu() {
    ...
    auto renderer = Renderer(menu_with_event, [&] {
        return window(text(" QUAN LY PHIEU THUE "), menu_with_event->Render() | vscroll_indicator | frame) | bold;
    });

    while (true) {
        system("cls");
        screen.Loop(renderer);
        if (selected == 0) { handle_view_rentals(); } 
        else if (selected == 1) { render_new_rental_screen(); } 
        else if (selected == 2) { render_return_comic_screen(); } 
        else if (selected == 3) { system("cls"); break; }
    }
}

...
// In ra chuỗi ngày chuẩn từ struct Date (dd/mm/yyyy)
...
void update_rental_status(const RentalSlip& updated_slip) {
    ...
    while (file.read(reinterpret_cast<char*>(&slip), sizeof(RentalSlip))) {
        if (slip.id_phieu == updated_slip.id_phieu) {
            file.seekp(static_cast<std::streampos>(file.tellg()) - static_cast<std::streampos>(sizeof(RentalSlip)));
            file.write(reinterpret_cast<const char*>(&updated_slip), sizeof(RentalSlip));
            break;
        }
    }
    file.close();
